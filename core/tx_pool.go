...
type TxPoolConfig struct {
	...
	// 允许进入txpool的最低gas price, 默认是1 gwei
	PriceLimit uint64 // Minimum gas price to enforce for acceptance into the pool
	/*
		priceBump：如果出现两个Nonce相同的交易，
		判断gasprice之间的差值，如果差值超过阈值则使用新交易替换老的交易
		如果旧交易a和新交易b之前的gas price差值是100
		而阈值accountSlots是90，则使用b替换掉a
	*/
	PriceBump  uint64 // Minimum price bump percentage to replace an already existing transaction (nonce)
	// pending 中每个账户所存储的交易数量的阈值，超出这个数量的交易可能会被认为是
	// 垃圾交易或者 攻击者从而被丢弃
...
func (pool *TxPool) validateTx(tx *types.Transaction, local bool) error {
	...
	// 交易的gas price必须大于pool设定的最低的gas price
	...
	// cost == V + GP * GL
	// 发送者当前的账户余额必须要大于 “交易金额+gasprice*gaslimit”
	if pool.currentState.GetBalance(from).Cmp(tx.Cost()) < 0 {
		return ErrInsufficientFunds
	}
	// 交易的gas limit必须要大于对应数据量所需的最低的gas
...
func (pool *TxPool) add(tx *types.Transaction, local bool) (bool, error) {
	...
	// 如果txpool满了，就删除gas price比较低的交易
	// 具体删除机制如下 ：
	// 	将当前的交易的gas price与txpool中gas price最低的交易进行比较，
	// 如果当前交易的gas price小于txpool中gas price最低的交易，就丢弃当前交易
	// 否则从txpool中删除比当前交易的gas price还要低的交易
...
func (pool *TxPool) promoteTx(addr common.Address, hash common.Hash, tx *types.Transaction) {
	...
	// 在pending中不存在给定账户的交易列表(有可能该账户是第一次发送)
...
func (pool *TxPool) promoteExecutables(accounts []common.Address) {
	...
	if pending > pool.config.GlobalSlots {
		...
		// 总结：上面代友其实就是对每个账户的交易数量做了判断处理
		/*
			如果有些账户的交易数量超过了账户交易数量限制，按照交易数量少的账户进行均衡
			如果有10个账户交易数量超过了AccountSlots(默认16)
			其中交易数量最少的账户包含了20笔交易，先把其它9个账户的交易数量消减到20
			如果pending长度仍然超过阈值，直接把这个账户的交易数量减到默认值16
		*/
